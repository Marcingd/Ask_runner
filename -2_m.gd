...
const FRAGMENT_FOLDER = "res://Map_Segment/-1/"
...
func generate_fragment():    
	...
	self.add_child(fragment_instance)  # Zmiana na self.add_child
...
func check_and_remove_fragments():
	...
	while i < created_fragments.size():
		...
		if end_marker.global_position.x + REMOVE_DISTANCE < player.position.x:
			...
			self.remove_child(fragment)  # Zmiana na self.remove_child
	...
	if last_fragment_end_marker.global_position.x - (REMOVE_DISTANCE * 2) < player.position.x:
		generate_fragment()
